...
public class TerminalStorageTabIngredientComponentItemStackCraftingCommon
    ...
    public List<Slot> loadSlots(Container container, int startIndex, EntityPlayer player,
        ...
        NonNullList<ItemStack> tabItems;

        if (partState != null) {
            tabItems = partState.getNamedInventory(this.getName().toString());
        } else  {
            tabItems = ItemTerminalStoragePortable.getNamedInventory(((ContainerTerminalStorage) container).getItemStack(player), this.getName().toString());
        }
    ...
    public void updateCraftingResult(EntityPlayer player, Container container, PartTypeTerminalStorage.State partState) {
        ...
        if (partState != null) {
            ...
            ItemTerminalStoragePortable.setNamedInventory(((ContainerTerminalStorage) container).getItemStack(player), this.getName().toString(), latestItems);
        }
    }
}
