    ...
    public ContainerTerminalStorage(EntityPlayer player, int itemIndex) {
        ...
        for (ITerminalStorageTab tab : TerminalStorageTabs.REGISTRY.getTabs()) {
            ...
            if (this.world.isRemote) {
                this.tabsClient.put(id, tab.createClientTab(this, player, null));
            } else {
                this.tabsServer.put(id, tab.createServerTab(this, player, this.getItemStack(player)));
            }
            ITerminalStorageTabCommon commonTab = tab.createCommonTab(this, player, null);
            if (commonTab != null) {
                this.tabsCommon.put(id, commonTab);

                int slotStartIndex = this.inventorySlots.size();
                List<Slot> slots = commonTab.loadSlots(this, slotStartIndex, player, null);
